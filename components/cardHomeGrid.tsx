...
export const CardHomeGrid = (ofertas: any) => {
  ...
  const cardIndexReset = () => {
    cardsPaginated.length === 0
      ? setCardIndex(totalPages - 1)
      : setCardIndex(cardIndex);
  };
  ...
  const paginateCards = () => {
    if (windowWidth >= 948) {
      setCardLimit(4);
    } else if (windowWidth <= 948 && windowWidth >= 717) {
      setCardLimit(3);
    } else if (windowWidth <= 717 && windowWidth >= 486) {
      setCardLimit(2);
    } else {
      setCardLimit(1);
    }
    ...
    <div className={styles.wrapper}>
      <div className={styles.title}>
        <div>
          ...
          <span className={styles.link} onClick={() => router.push("/ofertas")}>
            Ver todas
          </span>
        </div>
        <div className={styles.control}>
          <button onClick={cardIndexRewind}>
            <Image src={left} width="20" height="20" />
          </button>
          <button onClick={cardIndexForward}>
            <Image src={right} width="20" height="20" />
          </button>
        </div>
      </div>
      <div className={styles.container}>
        <div className={styles.controlSmall}>
          <button onClick={cardIndexRewind}>
            <Image src={left} width="20" height="20" />
          </button>
          <button onClick={cardIndexForward}>
